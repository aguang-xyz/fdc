...
typedef struct {         /* simplex solver control parameters */
...
void glp_check_kkt(glp_prob *P, int sol, int cond, double *ae_max, int *ae_ind,
...
int glp_ios_add_row(glp_tree *T, const char *name, int klass, int flags,
                    int len, const int ind[], const double val[], int type,
                    double rhs);
...
#ifdef GLP_UNDOC
int glp_gmi_cut(glp_prob *P, int j, int ind[], double val[], double phi[]);
...
void glp_mincost_lp(glp_prob *P, glp_graph *G, int names, int v_rhs, int a_low,
